...
describe Graphene::Memo do
  ...
  context 'shared_secret' do
    ...
    it 'should get shared_secret' do
      @data.each do |data|
        priv   = Graphene::Key.from_base58(data[0])
        pub    = data[1]
        secret = data[2]

        expect(Graphene::Memo.get_shared_secret(priv,pub)).to eq secret
      end
    end
  end
  ...
  context 'encrypt/decrypt' do
    ...
    it 'encrypt' do
      @data.each do |memo|
        encoded = Graphene::Memo.encode_memo memo[:wif], memo[:to], memo[:nonce], memo[:plain]

        expect(encoded).to eq memo[:message]
      end
    end

    it 'decrypt' do
      @data.each do |memo|
        decoded = Graphene::Memo.decode_memo memo[:wif], memo[:to], memo[:nonce], memo[:message]

        expect(decoded).to eq memo[:plain]
      end
    end
  end
