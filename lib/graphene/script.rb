...
class Graphene::Script
  ...
  def initialize(input_script, previous_output_script=nil)
    ...
    @raw = if @previous_output_script
             @input_script + [ Graphene::Script::OP_CODESEPARATOR ].pack("C") + @previous_output_script
           else
             @input_script
           end
    ...
    if previous_output_script
      ...
      @chunks << Graphene::Script::OP_CODESEPARATOR
      ...
      when Fixnum
        ...
        when *OPCODES_METHOD.keys
          ...
          when -2 # One fixed parameter, one optional
            ...
            puts "Graphene::Script: opcode #{name} method parameters invalid"
          end
        when *OP_2_16
          ...
          puts "Graphene::Script: opcode #{name} unkown or not implemented\n#{to_string.inspect}"
  ...
  def pay_to_script_hash(block_timestamp, opts, check_callback)
    ...
    *rest, script, _, script_hash, _ = @chunks
    ...
    return false unless Graphene.hash160(script.unpack("H*")[0]) == script_hash.unpack("H*")[0]
  ...
  def inner_p2sh_script
    ...
    *rest, script, _, script_hash, _ = @chunks
    ...
    return nil unless Graphene.hash160(script.unpack("H*")[0]) == script_hash.unpack("H*")[0]
    script
  end
  ...
  def is_pay_to_script_hash?
    ...
    if @previous_output_script
      chunks = Graphene::Script.new(@previous_output_script).chunks
      chunks.size == 3 &&
      chunks[-3] == OP_HASH160 &&
  ...
  def get_pubkey_address
    Graphene.pubkey_to_address(get_pubkey)
  end
  ...
  def get_hash160
    ...
    return Graphene.hash160(get_pubkey)        if is_pubkey?
  end

  # get the hash160 address for this hash160 script
  def get_hash160_address
    Graphene.hash160_to_address(get_hash160)
  end
  ...
  def get_multisig_addresses
    get_multisig_pubkeys.map{|pub|
      begin
        Graphene::Key.new(nil, pub.unpack("H*")[0]).addr
  ...
  def get_p2sh_address
    Graphene.hash160_to_p2sh_address(get_hash160)
  end
  ...
  def self.to_address_script(address)
    hash160 = Graphene.hash160_from_address(address)
    case Graphene.address_type(address)
  ...
  def self.to_p2sh_multisig_script(*args)
    ...
    p2sh_script = to_p2sh_script(Graphene.hash160(redeem_script.hth))
  ...
  def self.sort_p2sh_multisig_signatures script_sig, sig_hash
    ...
    sigs = Hash[script.chunks[1...-1].map.with_index do |sig, idx|
      pubkey = pubkeys.map {|key|
        Graphene::Key.new(nil, key.hth).verify(sig_hash, sig) ? key : nil }.compact.first
    ...
    @chunks.each do |chunk| # pushdate or opcode
      if chunk == OP_CHECKSIG || chunk == OP_CHECKSIGVERIFY
        ...
        if is_accurate && last_opcode && last_opcode.is_a?(Fixnum) && last_opcode >= OP_1 && last_opcode <= OP_16
          count += ::Graphene::Script.decode_OP_N(last_opcode)
        else
          count += 20
        end
      end
  ...
  # Graphened has somewhat special way to return count for invalid input scripts:
  ...
  def sigops_count_for_p2sh
    ...
    ::Graphene::Script.new(data).sigops_count_accurate(true)
  end
  ...
  def codehash_script(opcode)
    ...
    checkhash = Graphene.hash160(Graphene::Script.binary_from_string(script).unpack("H*")[0])
    [script, checkhash]
  end
  ...
  def op_checksig(check_callback, opts={})
    ...
    return (@stack << 0) unless Graphene::Script.check_pubkey_encoding?(pubkey, opts)
    drop_sigs      = [ cast_to_string(@stack[-1]) ]

    signature = cast_to_string(@stack.pop)
    return invalid unless Graphene::Script.check_signature_encoding?(signature, opts)
  ...
  def sighash_subscript(drop_sigs)
    if inner_p2sh? && @inner_script_code
      ::Graphene::Script.new(@inner_script_code).to_binary_without_signatures(drop_sigs)
    else
  ...
  def op_checkmultisig(check_callback, opts={})
    ...
    # Graphene-core removes an extra item from the stack
    @stack.pop
    ...
    while success && n_sigs > 0
      ...
      return (@stack << 0) unless Graphene::Script.check_pubkey_encoding?(pub, opts)
      return invalid unless Graphene::Script.check_signature_encoding?(sig, opts)
